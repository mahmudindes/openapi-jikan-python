...
REVIEWSCOLLECTIONDATAINNER_ANY_OF_SCHEMAS = ["AnimeReview", "MangaReview"]

class ReviewsCollectionDataInner(BaseModel):
    """
    ReviewsCollectionDataInner
    """

    # data type: AnimeReview
    anyof_schema_1_validator: Optional[AnimeReview] = None
    # data type: MangaReview
    anyof_schema_2_validator: Optional[MangaReview] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[AnimeReview, MangaReview]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: List[str] = Field(default=Literal["AnimeReview", "MangaReview"])
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ReviewsCollectionDataInner.model_construct()
        error_messages = []
        # validate data type: AnimeReview
        if not isinstance(v, AnimeReview):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AnimeReview`")
        else:
            return v

        # validate data type: MangaReview
        if not isinstance(v, MangaReview):
            error_messages.append(f"Error! Input type `{type(v)}` is not `MangaReview`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ReviewsCollectionDataInner with anyOf schemas: AnimeReview, MangaReview. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[AnimeReview] = None
        try:
            instance.actual_instance = AnimeReview.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[MangaReview] = None
        try:
            instance.actual_instance = MangaReview.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ReviewsCollectionDataInner with anyOf schemas: AnimeReview, MangaReview. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AnimeReview, MangaReview]]:
