...
RECOMMENDATIONSALLOFENTRY_ANY_OF_SCHEMAS = ["AnimeMeta", "MangaMeta"]

class RecommendationsAllOfEntry(BaseModel):
    """
    RecommendationsAllOfEntry
    """

    # data type: AnimeMeta
    anyof_schema_1_validator: Optional[AnimeMeta] = None
    # data type: MangaMeta
    anyof_schema_2_validator: Optional[MangaMeta] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[AnimeMeta, MangaMeta]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: List[str] = Field(default=Literal["AnimeMeta", "MangaMeta"])
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = RecommendationsAllOfEntry.model_construct()
        error_messages = []
        # validate data type: AnimeMeta
        if not isinstance(v, AnimeMeta):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AnimeMeta`")
        else:
            return v

        # validate data type: MangaMeta
        if not isinstance(v, MangaMeta):
            error_messages.append(f"Error! Input type `{type(v)}` is not `MangaMeta`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in RecommendationsAllOfEntry with anyOf schemas: AnimeMeta, MangaMeta. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[AnimeMeta] = None
        try:
            instance.actual_instance = AnimeMeta.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[MangaMeta] = None
        try:
            instance.actual_instance = MangaMeta.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into RecommendationsAllOfEntry with anyOf schemas: AnimeMeta, MangaMeta. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AnimeMeta, MangaMeta]]:
