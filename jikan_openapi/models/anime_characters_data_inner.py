...
class AnimeCharactersDataInner(BaseModel):
    """
    AnimeCharactersDataInner
    """ # noqa: E501
    character: Optional[AnimeCharactersDataInnerCharacter] = None
    role: Optional[StrictStr] = Field(default=None, description="Character's Role")
    voice_actors: Optional[List[AnimeCharactersDataInnerVoiceActorsInner]] = None
    __properties: ClassVar[List[str]] = ["character", "role", "voice_actors"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in voice_actors (list)
        _items = []
        if self.voice_actors:
            for _item in self.voice_actors:
                if _item:
                    _items.append(_item.to_dict())
            _dict['voice_actors'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "character": AnimeCharactersDataInnerCharacter.from_dict(obj["character"]) if obj.get("character") is not None else None,
            "role": obj.get("role"),
            "voice_actors": [AnimeCharactersDataInnerVoiceActorsInner.from_dict(_item) for _item in obj["voice_actors"]] if obj.get("voice_actors") is not None else None
        })
        return _obj
