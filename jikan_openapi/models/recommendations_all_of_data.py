...
class RecommendationsAllOfData(BaseModel):
    """
    RecommendationsAllOfData
    """ # noqa: E501
    mal_id: Optional[StrictStr] = Field(default=None, description="MAL IDs of recommendations is both of the MAL ID's with a `-` delimiter")
    entry: Optional[List[RecommendationsAllOfEntry]] = Field(default=None, description="Array of 2 entries that are being recommended to each other")
    content: Optional[StrictStr] = Field(default=None, description="Recommendation context provided by the user")
    user: Optional[UserById] = None
    __properties: ClassVar[List[str]] = ["mal_id", "entry", "content", "user"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "entry": [RecommendationsAllOfEntry.from_dict(_item) for _item in obj["entry"]] if obj.get("entry") is not None else None,
            "content": obj.get("content"),
            "user": UserById.from_dict(obj["user"]) if obj.get("user") is not None else None
        })
        return _obj
